...
func newConfigTemplate() configTemplate {
	return configTemplate{
		...
		BullyNetwork: make(map[uint]string),
...
// WithBullyNetwork sets up the network for bully
func WithBullyNetwork(network map[uint]string) Option {
	return func(ct *configTemplate) {
		ct.BullyNetwork = network
	}
}

// WithDisabledPKI sets up the network for bully
func WithDisabledPKI() Option {
	return func(ct *configTemplate) {
		ct.DisablePKI = true
	}
}
...
func NewTestNode(t *testing.T, f peer.Factory, trans transport.Transport,
	...
	config.DisablePKI = template.DisablePKI
...
// GetBully returns the Bully message associated to the transport.Message.
func GetBully(t *testing.T, msg *transport.Message) types.BullyMessage {
	require.Equal(t, "bully", msg.Type)

	var bullyMessage types.BullyMessage

	err := json.Unmarshal(msg.Payload, &bullyMessage)
	require.NoError(t, err)
